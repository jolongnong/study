...
public class BoardDAO{
	...
	public void insertArticle(BoardDTO article) throws Exception {
		...
		try {
			...
			sql = "insert into freeboard(num,writer,email,subject,passwd,reg_date,";
			sql+="ref,re_step,re_level,content,ip) values(freeboard_seq.NEXTVAL,?,?,?,?,?,?,?,?,?,?)";
				pstmt = conn.prepareStatement(sql);
	...
	public int getMyArticleCount(String id) throws Exception {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement("select count(*) from freeboard where writer=?");
	...
	public List getMyArticles(String id) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List articleList=null;
		try {
			...
			pstmt = conn.prepareStatement(
					...
					+ " where writer=?  order by reg_date desc");
					pstmt.setString(1, id); 
					 rs = pstmt.executeQuery();
					if (rs.next()) {
	...
	public String getWriter(int num) throws Exception {
	      ...
	      try {
	         conn = getConnection();
	         pstmt = conn.prepareStatement(" select writer from freeboard "
	               + " where ref=(select ref from freeboard where num=?) "
	               + " and re_step=0");
	         pstmt.setInt(1, num);
	            rs = pstmt.executeQuery();
	...
	public BoardDTO getArticle(int num) throws Exception {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(
			"update freeboard set readcount=readcount+1 where num = ?"); 
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
			pstmt = conn.prepareStatement(
	...
	public int updateArticle(BoardDTO article) throws Exception {
		...
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(
			"select passwd from freeboard where num = ?");
	...
	public int deleteArticle(int num, String passwd) throws Exception {
		...
		try {
				...
				if(passwd == null) {
				pstmt = conn.prepareStatement(
						"delete from freeboard where num=?");
				pstmt.setInt(1, num);					
				x = pstmt.executeUpdate();
				}else {
					pstmt = conn.prepareStatement(
							"delete from freeboard where num=? and passwd=?");
						pstmt.setInt(1, num);
